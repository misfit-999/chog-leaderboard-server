// server.js - Postgres-backed leaderboard (force IPv4 by resolving host first)
...
let pool; // will be created after hostname resolution

// Helper: parse URI and create Pool using IPv4 host if possible
async function createPoolFromConnectionString(uri) {
  try {
    const u = new URL(uri);
    ...
    // Try to resolve an IPv4 address for the hostname
    let hostToUse = originalHost;
    try {
      const lookupRes = await dns.lookup(originalHost, { family: 4 });
      if (lookupRes && lookupRes.address) {
        hostToUse = lookupRes.address;
        console.log(`Resolved ${originalHost} -> IPv4 ${hostToUse}`);
      } else {
        console.log(`dns.lookup returned no IPv4 for ${originalHost}, using hostname`);
      }
    } catch (dnsErr) {
      console.warn(`Could not resolve IPv4 for ${originalHost}:`, dnsErr.message || dnsErr);
      console.warn('Falling back to hostname (may attempt IPv6).');
    ...
    const poolConfig = {
      ...
      // family hint (some pg versions respect it)
      family: 4
    };
    ...
    console.error('createPoolFromConnectionString error', err);
    throw err;
  }
}

// Initialize DB and start server inside an async IIFE
(async () => {
  try {
    pool = await createPoolFromConnectionString(connectionString);
  ...
  // Start server only after DB init succeeded
